...
var urlBase2 = '?id=&jenis=&user_entry=&mid=&sub_jenis=&tgl_sort=tgl_entry&tid=&kanwil=I&tgl_awal=2022-06-01&tgl_akhir=2022-06-30&status=&pemasang=&submit=Generate';
...
async function getData(url, page) {
    ...
    let data = await page.evaluate(() => {
        ...
        items.forEach((item) => {
            results.push([
                item.querySelector("td:nth-child(1)") && item.querySelector("td:nth-child(1)").textContent,
                item.querySelector("td:nth-child(2)") && item.querySelector("td:nth-child(2)").textContent,
...
con.connect(async function (err) {
    ...
    var sql = 'INSERT INTO data_edc (ticket_id, merchant, mid, tid, peruntukan, entry_ticket, update_ticket, jenis_masalah, status, kanwil, pemasang) VALUES ?';
    // var sql2 = `
